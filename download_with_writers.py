...
def fetch_and_save_articles(resume_from_page=None):
    ...
    Then save them to a JSON Lines file in a new folder named 'articles_2024'.
    Also includes author IDs for each article.
    ...
    folder_name = "articles_2023_with_writers"
    if not os.path.exists(folder_name):
        os.makedirs(folder_name)
    
    # 2. Prepare the output file paths (JSON Lines)
    # Main output file that will contain all articles
    output_file = os.path.join(folder_name, "all_articles_w_writers.jsonl")
    ...
    #    - Added authorships to the select parameter to get author information
    params = {
        "filter": "publication_year:2023,institutions.country_code:us,cited_by_count:>3",
        "select": "display_name,topics,publication_date,authorships", 
    ...
    with open(output_file, main_file_mode, encoding="utf-8") as main_file:
        ...
        while has_more_pages:
            ...
            with open(iteration_file, "w", encoding="utf-8") as iter_file:
                ...
                for work in results:
                    # Extract author IDs for easier access if needed
                    if "authorships" in work:
                        author_ids = [authorship.get("author", {}).get("id") for authorship in work.get("authorships", [])]
                        work["author_ids"] = author_ids
